...
<table class="vertical-table">
    <tr>
        ...
        <td><?= $biditem->has('user') ? $biditem->user->username : '' ?>
        </td>
    </tr>
    <tr>
        <th scope="row">商品名</th>
    ...
    <tr>
        ...
        <td><?= $this->Number->format($biditem->id) ?>
        </td>
    </tr>
    <tr>
...
<div class="related">
    ...
    <?php if (!empty($biditem->bidinfo)) : ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <tr>
                <td><?= h($biditem->bidinfo->user->username) ?>
                </td>
...
<div class="related">
    ...
    <?php if (!$biditem->finished) : ?>
        <h6><a href="<?= $this->Url->build(['action' => 'bid', $biditem->id]) ?>">《入札する！》</a>
        </h6>
            ...
            <table cellpadding="0" cellspacing="0">
                ...
                <tbody>
                    <?php foreach ($bidrequests as $bidrequest) : ?>
                        <tr>
                            <td><?= h($bidrequest->user->username) ?>
                            </td>
                            <td><?= h($bidrequest->price) ?>円</td>
                            <td><?= $bidrequest->created ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
